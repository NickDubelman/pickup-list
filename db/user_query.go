...
func (uq *UserQuery) QueryOwnedLists() *ListQuery {
...
// the "owned_lists" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithOwnedLists(opts ...func(*ListQuery)) *UserQuery {
	query := &ListQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withOwnedLists = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withOwnedLists; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.OwnedLists = []*List{}
		}
		query.withFKs = true
		query.Where(predicate.List(func(s *sql.Selector) {
			s.Where(sql.InValues(user.OwnedListsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.list_owner
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "list_owner" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "list_owner" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OwnedLists = append(node.Edges.OwnedLists, n)
		}
	}
	...
	if query := uq.withLists; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.ListsPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "lists": %w`, err)
		}
		query.Where(list.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "lists" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Lists = append(nodes[i].Edges.Lists, n)
			}
		}
	}
