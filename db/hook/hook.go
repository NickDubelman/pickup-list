...
type ListFunc func(context.Context, *db.ListMutation) (db.Value, error)

// Mutate calls f(ctx, m).
func (f ListFunc) Mutate(ctx context.Context, m db.Mutation) (db.Value, error) {
	mv, ok := m.(*db.ListMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *db.ListMutation", m)
	}
	return f(ctx, mv)
}
